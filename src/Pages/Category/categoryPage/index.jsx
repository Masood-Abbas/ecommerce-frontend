...
const CategoryPage = () => {
  ...
  const fetchCategory = async (catId = selectedCategory) => {
    const params = {
      page,
      limit,
      minPrice: filters.minPrice,
  ...
  useEffect(() => {
    const minPrice = Number(searchParams.get("minPrice")) || 0;
    const maxPrice = Number(searchParams.get("maxPrice")) || 5000;
    const urlCategory = searchParams.get("category") || id;

    setFilters((prev) => ({ ...prev, minPrice, maxPrice }));
    setSelectedCategory(urlCategory);

    if (urlCategory) fetchCategory(urlCategory);
  }, [searchParams, id, page, limit]);
    ...
    <div className="main-container py-10 grid grid-cols-12 gap-8">
      ...
      <FilterSidebar
        ...
        setSelectedCategory={(catId) => {
          setSelectedCategory(catId);
          navigate(
            `/category/${catId}?page=1&limit=${limit}&minPrice=${filters.minPrice}&maxPrice=${filters.maxPrice}`
          );
        }}
        filters={filters}
